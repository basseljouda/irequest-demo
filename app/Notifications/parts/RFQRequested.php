...
class RFQRequested extends Notification implements ShouldQueue {
    ...
    public function toMail($notifiable) {
        $details = $this->partRequest->details->map(function ($detail) {
                    return $detail->part_title . ' - <b>Quantity</b>: ' . $detail->qty .
                            ' - <b>Condition:</b> ' . ucwords($detail->price_type) . ' - <b>Unit Price:</b> --';
                })->implode('<br>');

        return (new MailMessage)
                        ->subject('New RFQ Request: #' . $this->partRequest->id)
                        ->greeting(__('email.hello') . ' ' . ucwords($notifiable->name) . '!')
                        ->line('New RFQ requested by: ' . 
                                ucwords($this->partRequest->rfqRequestedBy->name))
    ...
    public function toNexmo($notifiable) {
        return (new NexmoMessage)
                        ->content("New RFQ requested by: " . ucwords($this->partRequest->rfqRequestedBy->name))->unicode();
    }
