...
class RMANewOrder extends Notification implements ShouldQueue {
    ...
    public function toMail($notifiable) {
        ...
        return (new MailMessage)
                        ->subject('New RMA for order: #' . $this->rma->order->getCustomID())
                        ->greeting(__('email.hello') . ' ' . ucwords($notifiable->name) . '!')
                        ->line('New RMA ('.$this->rma->id. ') requested for Order: #' . $this->rma->order->getCustomID() . ' by ' .
    ...
    public function toNexmo($notifiable) {
        return (new NexmoMessage)
                        ->content("New RMA submited #" . $this->rma->id . ' - ' . ucwords($this->rma->order->site->name))->unicode();
    }
