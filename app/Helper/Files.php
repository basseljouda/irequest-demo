...
class Files {
     ...
     * @throws \Exception
     */
    public static function uploadSignature($aname, $image) {
        ...
        $folderPath = public_path('user-uploads/' . $folder);
        ...
        $signature = self::generateNewFileName($aname . '.' . $image_type);
    ...
    public static function getNewOrderNo() {
        ...
        $jsonString = file_get_contents(base_path('resources/lang/ordersno.json'));

        $data = json_decode($jsonString, true);

        $orderNo = $data['last_order_no'] + 1;

        $data['last_order_no'] = $orderNo;

        $newJsonString = json_encode($data, JSON_PRETTY_PRINT);

        file_put_contents(base_path('resources/lang/ordersno.json'), stripslashes($newJsonString));

        return $orderNo;
    }
    
    public static function uploadFile($uploadedFile, $folder) {
        ...
        $uploadedFile->move(public_path('user-uploads/'.$folder), $newName);
